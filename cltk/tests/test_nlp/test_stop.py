...
class TestStop_General(unittest.TestCase):
    ...
    def test_corpus_stop_list_freq(self):
        ...
        stoplist = S.build_stoplist(self.test_latin_corpus, size=10,
    ...
    def test_corpus_stop_list_freq_include(self):
        ...
        S = LatinCorpusStoplist()
        stoplist = S.build_stoplist(self.test_latin_corpus, size=10, basis='frequency', include=['est'])
        self.assertEqual(stoplist, target_list)
    ...
    def test_corpus_stop_list_variance(self):
        ...
        target_list = ['ac', 'atque', 'cum', 'et', 'in', 'mihi', 'neque', 'qui', 'rerum', 'vel']
        S = LatinCorpusStoplist()
        stoplist = S.build_stoplist(self.test_latin_corpus, size=10,basis='variance')
        self.assertEqual(stoplist, target_list)
...
class TestStop_LanguageSpecific(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        Set up sample texts/corpus for testing stop.py
        """
        self.cc_1 = """方廣錩〔題解〕《天竺國菩提達摩禪師論》，又名《達摩禪師論》，中國僧人假託禪宗初祖菩提達摩所撰典籍，著者不詳，一卷。在敦煌遺書中，"""

        self.cc_2 = """至今已經發現兩種題名為《達摩禪師論》的文獻。其一為日本橋本凝胤所藏，首殘尾存，尾題作「達摩禪師論」，係唐高宗開耀元年"""

        self.cc_test_corpus = [self.cc_1, self.cc_2]

    def test_corpus_stop_list_freq_classical_chinese(self):
        """Test production of stoplists from a corpus, using basis: frequency"""
        target_list = ['一', '中', '名', '國', '師', '摩', '禪', '論', '達', '題']
        S = ClassicalChineseCorpusStoplist()
        stoplist = S.build_stoplist(self.cc_test_corpus, size=10,
                    basis='frequency', inc_values=False)
        self.assertEqual(stoplist, target_list)
